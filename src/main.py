...
try:
    ...
    send_notification("Fabric Launcher", "Fabric Launcher is not installed\n```bash\nyay -S python-fabric-git\n```", urgency="critical", app_name="EXS Launcher")
    exit(1)
...
def handle_exit(signum, frame):
    send_notification("Fabric Launcher", "Launcher disabled\n", urgency="low", app_name="EXS Launcher")
    exit(0)
...
if __name__ == "__main__":
    try:
        ...
        if reload:
            send_notification("Fabric Launcher", "Launcher reloaded\n", urgency="low", app_name="EXS Launcher")
        app.run()
    except (KeyboardInterrupt, SystemExit, OSError):
        handle_exit(0, None)
    except Exception:
        import traceback
        send_notification("Fabric Launcher", f"Fabric Launcher is not running\n```py\n{traceback.format_exc()}\n```", urgency="critical", app_name="EXS Launcher")
        exit(1)
